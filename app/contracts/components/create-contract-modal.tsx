...
export function CreateContractModal({ isOpen, onClose, onSuccess }: CreateContractModalProps) {
  ...
  const [uploadFormData, setUploadFormData] = useState<UploadExistContractRequest & { selectedFile: File | null }>({
    title: '',
    description: '',
    customerId: 0,
    contractDetails: [createEmptyContractDetail()],
  ...
  // Sync delivery schedule for upload form when devices are loaded or contract details change
  useEffect(() => {
    if (devices.length > 0 && uploadFormData.contractDetails.length > 0) {
      syncUploadDeliveryScheduleWithContractDetails()
    }
  }, [devices, uploadFormData.contractDetails])
  ...
  function handleUploadDetailChange(index: number, field: string, value: string | number) {
  ...
  function addUploadContractDetail() {
    setUploadFormData(prev => ({
      ...prev,
      contractDetails: [...(prev.contractDetails || []), createEmptyContractDetail()]
    }))
  ...
  function removeUploadContractDetail(index: number) {
    if ((uploadFormData.contractDetails?.length || 0) <= 1) return
  ...
  function handleUploadFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setUploadFormData(prev => ({ ...prev, selectedFile: file, filePath: '' }))
  }
  ...
  function handleUploadDeliveryScheduleChange(index: number, field: keyof DeliverySchedule, value: string | number) {
    const updatedSchedules = [...uploadFormData.deliverySchedules]
    updatedSchedules[index] = {
      ...updatedSchedules[index],
      [field]: value
    }
    setUploadFormData({ ...uploadFormData, deliverySchedules: updatedSchedules })
  }
  ...
  // Function to sync delivery schedule with contract details for upload form
  function syncUploadDeliveryScheduleWithContractDetails() {
    const updatedSchedules = uploadFormData.contractDetails.map((detail, index) => {
      if (detail.deviceId) {
        const selectedDevice = devices.find(d => d.id === detail.deviceId)
        if (selectedDevice) {
          return {
            ...
            unit: selectedDevice.unit || '',
            quantity: detail.quantity,
            deliveryTime: uploadFormData.deliverySchedules[index]?.deliveryTime || '',
            deliveryLocation: uploadFormData.deliverySchedules[index]?.deliveryLocation || '',
            notes: uploadFormData.deliverySchedules[index]?.notes || ''
          }
        }
      }
      // If no device selected, create empty schedule
  ...
  async function handleUploadSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      try {
        const tempContractNumber = `TEMP_${Date.now()}`
        filePath = await uploadContractFileApi(uploadFormData.selectedFile, tempContractNumber)
      } catch (err) {
        console.error('Error uploading file:', err)
        const errorMessage = err instanceof Error ? err.message : 'Failed to upload contract file'
        setFileError(errorMessage)
      ...
      // Calculate total value from contract details
      const totalValue = uploadFormData.contractDetails?.reduce(
        (sum, detail) => sum + (detail.quantity * (detail.unitPrice || 0)), 
        0
      ) || 0
      
      // Submit form data with the uploaded file path
      await uploadExistContract({
        ...uploadFormData,
        customerId: Number(uploadFormData.customerId),
        totalValue,
        filePath
      })
      ...
      console.error('Error uploading existing contract:', err)
      if (err instanceof Error) {
        setError(err.message)
      } else if (typeof err === 'string') {
        setError(err)
      } else {
        setError('Failed to upload existing contract. Please check your input and try again.')
      }
    } finally {
      setLoading(false)
    }
            ...
            <form onSubmit={handleUploadSubmit}>
              <div className="space-y-4">
                  ...
                  <div className="col-span-1 md:col-span-2">
                    ...
                    <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-md">
                      <p className="text-sm text-blue-800">
                        <strong>Lưu ý:</strong> Hợp đồng này sẽ được tạo với trạng thái "Đã ký và có hiệu lực" ngay lập tức. 
                        Các thiết bị trong hợp đồng sẽ được thêm vào "My Device" của khách hàng.
                      </p>
                    </div>
                      ...
                      {uploadFormData.selectedFile ? (
                        <div className="flex items-center gap-2 w-full">
                          ...
                          <div className="flex-1 truncate">
                            <p className="font-medium">{uploadFormData.selectedFile.name}</p>
                            <p className="text-xs text-muted-foreground">
                              {(uploadFormData.selectedFile.size / 1024 / 1024).toFixed(2)} MB
                            </p>
                          </div>
                          <button
                            type="button"
                            onClick={() => setUploadFormData(prev => ({ ...prev, selectedFile: null, filePath: '' }))}
                            className="p-1 text-red-500 hover:bg-red-50 rounded-full"
                          >
                            <X size={16} />
                          </button>
                ...
                <div className="space-y-4">
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full border border-gray-300">
                      ...
                      <tbody>
                        {uploadFormData.deliverySchedules.map((schedule, index) => (
                          <tr key={index}>
                            <td className="border border-gray-300 px-3 py-2 text-center text-sm">
                              {index + 1}
                            </td>
                            ...
                            <td className="border border-gray-300 px-3 py-2">
                              <input
                                type="text"
                                value={schedule.deliveryTime}
                                onChange={(e) => handleUploadDeliveryScheduleChange(index, 'deliveryTime', e.target.value)}
                                className="w-full p-1 border rounded text-sm"
                                placeholder="VD: 15 ngày"
                              />
                            </td>
                            <td className="border border-gray-300 px-3 py-2">
                              <textarea
                                value={schedule.deliveryLocation}
                                onChange={(e) => handleUploadDeliveryScheduleChange(index, 'deliveryLocation', e.target.value)}
                                className="w-full p-1 border rounded text-sm"
                                placeholder="Địa điểm giao hàng"
                                rows={2}
                              />
                            ...
                            <td className="border border-gray-300 px-3 py-2">
                              <textarea
                                value={schedule.notes}
                                onChange={(e) => handleUploadDeliveryScheduleChange(index, 'notes', e.target.value)}
                                className="w-full p-1 border rounded text-sm"
                                placeholder="Ghi chú"
                                rows={2}
                              />
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      type="button"
                      onClick={addUploadContractDetail}
                      className="flex items-center gap-1 text-sm text-primary hover:underline"
                    >
                      <Plus size={16} /> Add Item
                    </button>
                  ...
                  {uploadFormData.contractDetails?.map((detail, index) => (
                    <div key={index} className="border rounded-md p-4 mb-4">
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <button
                          type="button"
                          onClick={() => removeUploadContractDetail(index)}
                          className="p-1 text-red-500 hover:bg-red-50 rounded-full"
                          disabled={(uploadFormData.contractDetails?.length || 0) <= 1}
                        >
                          <Trash2 size={16} />
                        </button>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input
                            type="text"
                            value={detail.workCode}
                            onChange={(e) => handleUploadDetailChange(index, 'workCode', e.target.value)}
                            className="w-full border rounded-md p-2 focus:ring-2 focus:ring-primary"
                            required
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <select
                            value={detail.deviceId || ''}
                            onChange={(e) => handleUploadDetailChange(index, 'deviceId', Number(e.target.value))}
                            className="w-full border rounded-md p-2 focus:ring-2 focus:ring-primary"
                            disabled={devicesLoading}
                          >
                            <option value="">Select a device (optional)</option>
                        ...
                        <div>
                          ...
                          <textarea
                            value={detail.description || ''}
                            onChange={(e) => handleUploadDetailChange(index, 'description', e.target.value)}
                            className="w-full border rounded-md p-2 focus:ring-2 focus:ring-primary h-16"
                            placeholder="Description (optional)"
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            min="1"
                            value={detail.quantity}
                            onChange={(e) => handleUploadDetailChange(index, 'quantity', e.target.value)}
                            className="w-full border rounded-md p-2 focus:ring-2 focus:ring-primary"
                            required
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            min="0"
                            value={detail.warrantyMonths || 0}
                            onChange={(e) => handleUploadDetailChange(index, 'warrantyMonths', e.target.value)}
                            className="w-full border rounded-md p-2 focus:ring-2 focus:ring-primary"
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            value={detail.notes || ''}
                            onChange={(e) => handleUploadDetailChange(index, 'notes', e.target.value)}
                            className="w-full border rounded-md p-2 focus:ring-2 focus:ring-primary"
                            placeholder="Additional notes (optional)"
                          />
                        </div>
              ...
              <div className="mt-8 flex justify-end space-x-3">
                ...
                <button
                  ...
                  disabled={loading || uploadingFile}
                >
                  {loading || uploadingFile ? 'Creating...' : 'Create Active Contract from PDF'}
                </button>
              </div>
            </form>
          )}
